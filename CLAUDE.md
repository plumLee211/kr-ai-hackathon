...
- **Any task requiring multiple tool calls** (file reads, searches, bash commands, API calls) → delegate to a subagent or team. Tool-call-heavy work consumed in the main context inflates token usage and degrades reasoning quality.
...
**When given a bug report, just fix it. Don't ask the user how.**
