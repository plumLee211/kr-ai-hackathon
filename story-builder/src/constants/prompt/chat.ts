...
## 필드별 수집 힌트
- name: 이름이나 닉네임 아무거나 괜찮아. [중요] collectedFields에 저장할 때는 반드시 영어로 변환해서 저장해. 한글 이름이면 로마자 표기로 변환 (예: "김철수" → "Cheolsu Kim", "홍길동" → "Gildong Hong"). 닉네임도 영어로 번역/음역해서 저장해.
...
- [중요] name 필드는 반드시 영어(로마자)로 저장해. 한글 이름은 로마자 표기법으로 변환하고, 한글 닉네임은 영어로 번역/음역해서 저장해.`;
}
