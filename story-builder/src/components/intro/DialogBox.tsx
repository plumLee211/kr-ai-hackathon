...
export function DialogBox({ message, onEnter, isFinished, placeholder = "입력 후 Enter를 누르세요...", isLoading = false }: DialogBoxProps) {
  const [inputValue, setInputValue] = useState("");

  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    // 띄어쓰기 입력을 위해 e.nativeEvent.isComposing 체크 (한글 입력 시 두번 입력 방지)
    if (e.key === "Enter" && inputValue.trim() !== "" && !e.nativeEvent.isComposing && !isLoading) {
    ...
    <div className="relative z-10 border-2 border-gray-400 p-6 w-full max-w-2xl bg-black/80 font-mono flex flex-col gap-6 shadow-2xl">
      ...
      <div className="flex items-center text-green-400 text-xl">
        ...
        <input
          ...
          disabled={isLoading}
          className="bg-transparent outline-none flex-1 text-green-400 placeholder-green-800/50 disabled:opacity-50"
