 ...
 * @param name 캐릭터 이름
 */
export const spriteSheetPrompt = (name: string, direction: "front" | "back" | "side"): string => {
  const directionGuide = {
    front: "첫번째 행: 아래로 걷기 (정면, 캐릭터의 얼굴이 보임)",
    back: "두번째 행: 위로 걷기 (뒷면, 캐릭터의 등이 보임)",
    side: "세번째 행: 왼쪽으로 걷기 (측면, 캐릭터의 옆모습이 보임)"
  };

  return `이 캐릭터의 이름은 "${name}"입니다.

          이 캐릭터의 위/아래/좌로 걷는 도트 스프라이트 이미지를 생성해줘.

          요구사항:
          - 모던 픽셀 아트 스타일 (스타듀밸리, 언더테일 레퍼런스). 둥근 머리, 2~3등신 비율.
          - 2방향 걷기 애니메이션, 방향당 2프레임 = 총 6프레임.
          - **단일 스프라이트 시트 이미지**로 출력. 3행 2열 그리드.
          - 행 순서 (반드시 지켜야 함):
            - ${directionGuide[direction]} - exactly 2 frames in one row.
